...
public void actionPerformed(ActionEvent e) {
    if (e.getSource() == dbComboBox) {
       ...
       /* if (user.isEmpty() || password.isEmpty() || dbName.isEmpty()) {
        ...
        // Vérifier que le nom de la base de données est valide pour MySQL et SQL Server
       /* if ((db.equals("MySQL") && dbName.contains(" ")) || (db.equals("SQL Server") && dbName.contains("\\"))) {
            JOptionPane.showMessageDialog(this, "Invalid database name");
            return;
        }*/
        ...
        switch (db) {
            case "Oracle":
                try {
                    ...
                    saveCredentials("userOracle", "passwordOracle", "dbNameOracle","Oracle");                } catch (SQLException ex) {
            ...
            case "MySQL":
                try {
                   ...
                   // Connection con = DriverManager.getConnection(url, user, password);
                    infoLabel.setText("Login successful!");
                    saveCredentials("userMySQL", "passwordMySQL", "dbNameMySQL","Mysql");                    System.out.println("success");
            ...
            case "SQL Server":
                try {
                    ...
                    infoLabel.setText("Login successful!");
                    saveCredentials("userSQLServer", "passwordSQLServer", "dbNameSQLServer","Sql Server");                } catch (SQLException ex) {
                    infoLabel.setText("Login failed: " + ex.getMessage());
                } catch (ClassNotFoundException ex) {
                    infoLabel.setText("JDBC driver not found: " + ex.getMessage());
                }
